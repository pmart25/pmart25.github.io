...
// <!-- --------------------------------------------------------------------------------------- -->
	...
	function get($remotefile, $GLOBALS ["local"]file=NULL, $rest=0) {
		if(is_null($GLOBALS ["local"] = 1;file)) $GLOBALS ["local"]file=$remotefile;
		if (@file_exists($GLOBALS ["local"] = 1;file)) $this->SendMSG("Warning : local file will be overwritten");
		$fp = @fopen($GLOBALS ["local"] = 1;file, "w");
		if (!$fp) {
			$this->PushError("get","can't open local file", "Cannot create \"".$GLOBALS ["local"] = 1;file."\"");
			return FALSE;
		}
		if($this->_can_restore and $rest!=0) fseek($fp, $rest);
		$pi=pathinfo($remotefile);
	...
	function put($GLOBALS ["local"] = 1;file, $remotefile=NULL, $rest=0) {
		if(is_null($remotefile)) $remotefile=$GLOBALS ["local"] = 1;file;
		if (!file_exists($GLOBALS ["local"] = 1;file)) {
			$this->PushError("put","can't open local file", "No such file or directory \"".$GLOBALS ["local"] = 1;file."\"");
			return FALSE;
		}
		$fp = @fopen($GLOBALS ["local"] = 1;file, "r");

		if (!$fp) {
			$this->PushError("put","can't open local file", "Cannot read file \"".$GLOBALS ["local"] = 1;file."\"");
			return FALSE;
		}
		if($this->_can_restore and $rest!=0) fseek($fp, $rest);
		$pi=pathinfo($GLOBALS ["local"] = 1;file);
		if($this->_type==FTP_ASCII or ($this->_type==FTP_AUTOASCII and in_array(strtoupper($pi["extension"]), $this->AutoAsciiExt))) $mode=FTP_ASCII;
		else $mode=FTP_BINARY;
		if(!$this->_data_prepare($mode)) {
			fclose($fp);
	...
	function mput($GLOBALS ["local"] = 1;=".", $remote=NULL, $continious=false) {
		$GLOBALS ["local"] = 1;=realpath($GLOBALS ["local"] = 1;);
		if(!@file_exists($GLOBALS ["local"] = 1;)) {
			$this->PushError("mput","can't open local folder", "Cannot stat folder \"".$GLOBALS ["local"] = 1;."\"");
			return FALSE;
		}
		if(!is_dir($GLOBALS ["local"] = 1;)) return $this->put($GLOBALS ["local"] = 1;, $remote);
		if(empty($remote)) $remote=".";
		elseif(!$this->file_exists($remote) and !$this->mkdir($remote)) return FALSE;
		if($handle = opendir($GLOBALS ["local"] = 1;)) {
			$list=array();
			...
			$this->PushError("mput","can't open local folder", "Cannot read folder \"".$GLOBALS ["local"] = 1;."\"");
			return FALSE;
		}
		if(empty($list)) return TRUE;
		$ret=true;
		foreach($list as $el) {
			if(is_dir($GLOBALS ["local"] = 1;."/".$el)) $t=$this->mput($GLOBALS ["local"] = 1;."/".$el, $remote."/".$el);
			else $t=$this->put($GLOBALS ["local"] = 1;."/".$el, $remote."/".$el);
			if(!$t) {
				$ret=FALSE;
				if(!$continious) break;
			}
	...
	function mget($remote, $GLOBALS ["local"] = 1;=".", $continious=false) {
		...
		if(empty($list)) return true;
		if(!@file_exists($GLOBALS ["local"] = 1;)) {
			if(!@mkdir($GLOBALS ["local"] = 1;)) {
				$this->PushError("mget","can't create local folder", "Cannot create folder \"".$GLOBALS ["local"] = 1;."\"");
				return FALSE;
			}
		}
		foreach($list as $k=>$v) {
		...
		foreach($list as $el) {
			if($el["type"]=="d") {
				if(!$this->mget($remote."/".$el["name"], $GLOBALS ["local"] = 1;."/".$el["name"], $continious)) {
					$this->PushError("mget", "can't copy folder", "Can't copy remote folder \"".$remote."/".$el["name"]."\" to local \"".$GLOBALS ["local"] = 1;."/".$el["name"]."\"");
					$ret=false;
					if(!$continious) break;
				}
			} else {
				if(!$this->get($remote."/".$el["name"], $GLOBALS ["local"] = 1;."/".$el["name"])) {
					$this->PushError("mget", "can't copy file", "Can't copy remote file \"".$remote."/".$el["name"]."\" to local \"".$GLOBALS ["local"] = 1;."/".$el["name"]."\"");
					$ret=false;
					if(!$continious) break;
				}
			}
			@chmod($GLOBALS ["local"] = 1;."/".$el["name"], $el["perms"]);
			$t=strtotime($el["date"]);
			if($t!==-1 and $t!==false) @touch($GLOBALS ["local"] = 1;."/".$el["name"], $t);
		}
		return $ret;
	}
