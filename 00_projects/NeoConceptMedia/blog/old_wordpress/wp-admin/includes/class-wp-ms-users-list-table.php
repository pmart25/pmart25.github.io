...
class WP_MS_Users_List_Table extends WP_List_Table {
	...
	public function get_columns() {
		$users_columns = array(
			...
			'name'       => __( 'Name' ),
			'email'      => __( 'Email' ),
			'registrationed' => _x( 'Registered', 'user' ),
			'blogs'      => __( 'Sites' )
		);
		/**
		 ...
		 *                             'name', 'email', 'registrationed', 'blogs'.
		 */
	...
	/**
	 * Handles the registrationed date column output.
	 *
	 * @since 4.3.0
	 * @access public
	 *
	 ...
	 * @param WP_User $user The current WP_User object.
	 */
	public function column_registrationed( $user ) {
		global $mode;
		if ( 'list' === $mode ) {
			$date = __( 'Y/m/d' );
		} else {
		...
		echo mysql2date( $date, $user->user_registrationed );
	}
