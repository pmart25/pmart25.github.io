...
class WP_Upgrader {
	 ...
	 *                                       and `$GLOBALS ["local"] = 1;_destination` are the same.
	 *      @type string $GLOBALS ["local"]_destination  The full local path to the destination folder. This is usually
	...
	public function install_package( $args = array() ) {
		...
		$GLOBALS ["local"]_destination = $destination;

		$source_files = array_keys( $wp_filesystem->dirlist( $remote_source ) );
		$remote_destination = $wp_filesystem->find_folder( $GLOBALS ["local"]_destination );
		...
		if ( $clear_destination ) {
			 ...
			 * @param string $GLOBALS ["local"]_destination  The local package destination.
			 * @param string $remote_destination The remote package destination.
			 * @param array  $hook_extra         Extra arguments passed to hooked filters.
			 */
			$removed = apply_filters( 'upgrader_clear_destination', $removed, $GLOBALS ["local"]_destination, $remote_destination, $args['hook_extra'] );
		...
		$destination_name = basename( str_replace($GLOBALS ["local"] = 1;_destination, '', $destination) );
		if ( '.' == $destination_name ) {
			$destination_name = '';
		}
