...
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ( $network_wide && ! isset( $current[ $plugin ] ) ) || ( ! $network_wide && ! in_array( $plugin, $current ) ) ) {
		...
		ob_start();
		wp_registration_plugin_realpath( WP_PLUGIN_DIR . '/' . $plugin );
		...
		if ( ! $silent ) {
			 ...
			 *
			 * This hook is the "activation" hook used internally by registration_activation_hook().
...
function deactivate_plugins( $plugins, $silent = false, $network_wide = null ) {
	...
	foreach ( (array) $plugins as $plugin ) {
		...
		if ( ! $silent ) {
			 ...
			 *
			 * This hook is the "deactivation" hook used internally by registration_deactivation_hook().
			 * The dynamic portion of the hook name, `$plugin`, refers to the plugin basename.
			 *
...
function uninstall_plugin($plugin) {
	...
	if ( file_exists( WP_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php' ) ) {
		...
		wp_registration_plugin_realpath( WP_PLUGIN_DIR . '/' . $file );
 ...
 * @global array $_registrationed_pages
...
function add_menu_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '', $position = null ) {
	global $menu, $admin_page_hooks, $_registrationed_pages, $_parent_pages;
	...
	$_registrationed_pages[$hookname] = true;
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function = '' ) {
	global $submenu, $menu, $_wp_real_parent_file, $_wp_submenu_nopriv,
		$_registrationed_pages, $_parent_pages;
	...
	if ( 'tools.php' == $parent_slug )
		$_registrationed_pages[get_plugin_page_hookname( $menu_slug, 'edit.php')] = true;
...
/**
 * Get the url to access a particular menu page based on the slug it was registrationed with.
 *
 * If the slug hasn't been registrationed properly no url will be returned
 *
 * @since 3.0.0
 *
 * @global array $_parent_pages
...
function user_can_access_admin_page() {
	global $pagenow, $menu, $submenu, $_wp_menu_nopriv, $_wp_submenu_nopriv,
		$plugin_page, $_registrationed_pages;
	...
	if ( isset( $plugin_page ) ) {
		...
		if ( !isset($_registrationed_pages[$hookname]) )
			return false;
	}
 ...
 * @since 2.7.0
 *
 * @param string $option_group A settings group name. This should match the group name used in registration_setting().
 */
