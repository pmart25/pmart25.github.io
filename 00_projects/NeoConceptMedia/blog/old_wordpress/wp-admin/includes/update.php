 ...
 * @param string $GLOBALS ["local"]e  Locale to query.
 * @return bool|array False on failure. An array of checksums on success.
 */
function get_core_checksums( $version, $GLOBALS ["local"]e ) {
 ...
 * @param string $GLOBALS ["local"]e
 * @return bool
 */
function undismiss_core_update( $version, $GLOBALS ["local"]e ) {
	$dismissed = get_site_option( 'dismissed_update_core' );
	$key = $version . '|' . $GLOBALS ["local"]e;
 ...
 * @return object|false
 */
function find_core_update( $version, $GLOBALS ["local"]e ) {
	$from_api = get_site_transient( 'update_core' );
	...
	$updates = $from_api->updates;
	foreach ( $updates as $update ) {
		if ( $update->current == $version && $update->locale == $GLOBALS ["local"]e )
			return $update;
	}
	return false;
}
