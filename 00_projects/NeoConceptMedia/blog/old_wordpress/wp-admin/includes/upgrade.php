...
function upgrade_110() {
	...
	if (!$got_gmt_fields) {
		...
		$wpdb->query("UPDATE $wpdb->users SET user_registrationed = DATE_ADD(user_registrationed, INTERVAL '$add_hours:$add_minutes' HOUR_MINUTE)");
	}
...
function upgrade_330() {
	global $wp_current_db_version, $wpdb, $wp_registrationed_widgets, $sidebars_widgets;
	...
	switch ( $sidebars_widgets['array_version'] ) {
		case 1 :
			...
			foreach ( (array) $sidebar as $i => $name ) {
				$id = strtolower($name);
				if ( isset($wp_registrationed_widgets[$id]) ) {
