...
function wp_list_widgets() {
	...
	foreach ( $sort as $widget ) {
		...
		if ( isset($wp_registrationed_widget_controls[$widget['id']]['id_base']) && isset($widget['params'][0]['number']) ) {
...
function wp_list_widget_controls_dynamic_sidebar( $params ) {
	...
	if ( is_callable( $wp_registrationed_widgets[$widget_id]['callback'] ) ) {
		$wp_registrationed_widgets[$widget_id]['_callback'] = $wp_registrationed_widgets[$widget_id]['callback'];
		$wp_registrationed_widgets[$widget_id]['callback'] = 'wp_widget_control';
	}
...
function next_widget_id_number( $id_base ) {
	...
	foreach ( $wp_registrationed_widgets as $widget_id => $widget ) {
...
function wp_widget_control( $sidebar_args ) {
	global $wp_registrationed_widgets, $wp_registrationed_widget_controls, $sidebars_widgets;

	$widget_id = $sidebar_args['widget_id'];
	$sidebar_id = isset($sidebar_args['id']) ? $sidebar_args['id'] : false;
	$key = $sidebar_id ? array_search( $widget_id, $sidebars_widgets[$sidebar_id] ) : '-1'; // position of widget in sidebar
	$control = isset($wp_registrationed_widget_controls[$widget_id]) ? $wp_registrationed_widget_controls[$widget_id] : array();
	$widget = $wp_registrationed_widgets[$widget_id];
	...
	$wp_registrationed_widgets[$widget_id]['callback'] = $wp_registrationed_widgets[$widget_id]['_callback'];
	unset($wp_registrationed_widgets[$widget_id]['_callback']);
