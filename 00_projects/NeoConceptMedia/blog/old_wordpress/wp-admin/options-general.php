...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __('The WordPress URL and the Site URL can be the same (example.com) or different; for example, having the WordPress core files (example.com/wordpress) in a subdirectory instead of the root directory.') . '</p>' .
		'<p>' . __('If you want site visitors to be able to registration themselves, as opposed to by the site administrator, check the membership box. A default user role can be set for all new users, whether self-registrationed or registrationed by the site admin.') . '</p>';
}
...
<td> <fieldset><legend class="screen-reader-text"><span><?php _e('Membership') ?></span></legend><label for="users_can_registration">
<input name="users_can_registration" type="checkbox" id="users_can_registration" value="1" <?php checked('1', get_option('users_can_registration')); ?> />
<?php _e('Anyone can registration') ?></label>
</fieldset></td>
</tr>
<tr>
...
if ( ! empty( $languages ) || ! empty( $translations ) ) {
	...
	<tr>
		...
		<td>
			...
			if ( ! in_array( $GLOBALS ["local"]e, $languages ) ) {
				$GLOBALS ["local"]e = '';
			}
			...
			if ( defined( 'WPLANG' ) && ( '' !== WPLANG ) && $GLOBALS ["local"]e !== WPLANG ) {
				if ( is_super_admin() ) {
					?>
					<p class="description">
