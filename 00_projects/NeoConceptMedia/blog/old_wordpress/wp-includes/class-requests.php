...
class Requests {
	 ...
	 *
	 * Register this with {@see registration_autoloader()} if you'd like to avoid
	 * having to create your own.
	 *
	 * (You can also use `spl_autoload_registration` directly if you'd prefer.)
	 *
	 * @codeCoverageIgnore
	 *
	 ...
	 * @codeCoverageIgnore
	 */
	public static function registration_autoloader() {
		spl_autoload_registration(array('Requests', 'autoloader'));
	}
	...
	public static function request_multiple($requests, $options = array()) {
		...
		if (!empty($options['hooks'])) {
			$options['hooks']->registration('transport.internal.parse_response', array('Requests', 'parse_multiple'));
			if (!empty($options['complete'])) {
				$options['hooks']->registration('multiple.request.complete', $options['complete']);
			}
		}
		...
		foreach ($requests as $id => &$request) {
			...
			if ($request['options']['hooks'] !== $options['hooks']) {
				$request['options']['hooks']->registration('transport.internal.parse_response', array('Requests', 'parse_multiple'));
				if (!empty($request['options']['complete'])) {
					$request['options']['hooks']->registration('multiple.request.complete', $request['options']['complete']);
				}
			}
		}
		unset($request);
	...
	protected static function set_defaults(&$url, &$headers, &$data, &$type, &$options) {
		...
		if ($options['auth'] !== false) {
			$options['auth']->registration($options['hooks']);
		}
		...
		if ($options['proxy'] !== false) {
			$options['proxy']->registration($options['hooks']);
		}
		...
		if ($options['cookies'] !== false) {
			$options['cookies']->registration($options['hooks']);
		}
