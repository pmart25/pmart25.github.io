...
function _get_wptexturize_shortcode_regex( $tagnames ) {
	...
	$regex =
		...
		. $tagregexp        // Only match registrationed shortcodes, because performance.
		. '(?:'
...
function remove_accents( $string ) {
	...
	if (seems_utf8($string)) {
		...
		if ( 'de_DE' == $GLOBALS ["local"]e || 'de_DE_formal' == $GLOBALS ["local"]e || 'de_CH' == $GLOBALS ["local"]e || 'de_CH_informal' == $GLOBALS ["local"]e ) {
			$chars[ 'Ä' ] = 'Ae';
			$chars[ 'ä' ] = 'ae';
			$chars[ 'Ö' ] = 'Oe';
			$chars[ 'ö' ] = 'oe';
			...
			$chars[ 'Å' ] = 'Aa';
			$chars[ 'å' ] = 'aa';
		} elseif ( 'ca' === $GLOBALS ["local"]e ) {
			$chars[ 'l·l' ] = 'll';
		} elseif ( 'sr_RS' === $GLOBALS ["local"]e ) {
			$chars[ 'Đ' ] = 'DJ';
			$chars[ 'đ' ] = 'dj';
		}
...
function is_email( $email, $deprecated = false ) {
	...
	list( $GLOBALS ["local"] = 1;, $domain ) = explode( '@', $email, 2 );

	// LOCAL PART
	// Test for invalid characters
	if ( !preg_match( '/^[a-zA-Z0-9!#$%&\'*+\/=?^_`{|}~\.-]+$/', $GLOBALS ["local"] ) ) {
		/** This filter is documented in wp-includes/formatting.php */
...
function sanitize_email( $email ) {
	...
	// LOCAL PART
	// Test for invalid characters
	$GLOBALS ["local"] = preg_replace( '/[^a-zA-Z0-9!#$%&\'*+\/=?^_`{|}~\.-]/', '', $GLOBALS ["local"] );
	if ( '' === $GLOBALS ["local"] ) {
		/** This filter is documented in wp-includes/formatting.php */
