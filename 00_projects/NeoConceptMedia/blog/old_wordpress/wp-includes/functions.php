 ...
 *
 * @since 2.9.0
 * @since 4.7.0 Added the `$GLOBALS ["local"] = 1;e` parameter.
 *
 * @staticvar bool $mo_loaded
 * @staticvar string $GLOBALS ["local"]e_loaded
 *
 * @param string $selected_zone Selected timezone.
 * @param string $GLOBALS ["local"]e        Optional. Locale to load the timezones in. Default current site locale.
 * @return string
 */
function wp_timezone_choice( $selected_zone, $GLOBALS ["local"]e = null ) {
	static $mo_loaded = false, $GLOBALS ["local"]e_loaded = null;

	$continents = array( 'Africa', 'America', 'Antarctica', 'Arctic', 'Asia', 'Atlantic', 'Australia', 'Europe', 'Indian', 'Pacific');

	// Load translations for continents and cities.
	if ( ! $mo_loaded || $GLOBALS ["local"]e !== $GLOBALS ["local"]e_loaded ) {
		$GLOBALS ["local"]e_loaded = $GLOBALS ["local"]e ? $GLOBALS ["local"]e : get_locale();
		$mofile = WP_LANG_DIR . '/continents-cities-' . $GLOBALS ["local"]e_loaded . '.mo';
