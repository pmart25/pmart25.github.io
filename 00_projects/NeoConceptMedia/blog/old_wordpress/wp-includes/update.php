...
function wp_version_check( $extra_stats = array(), $force_check = false ) {
	 ...
	 * @since 2.8.0
	 *
	 * @param string $GLOBALS ["local"]e Current locale.
	 */
	$GLOBALS ["local"]e = apply_filters( 'core_version_check_locale', get_locale() );
...
function wp_update_plugins( $extra_stats = array() ) {
	...
	$GLOBALS ["local"]es = array_values( get_available_languages() );

	/**
	 * Filters the locales requested for plugin translations.
	 *
	 * @since 3.7.0
	 * @since 4.5.0 The default value of the `$GLOBALS ["local"] = 1;es` parameter changed to include all locales.
	 *
	 * @param array $GLOBALS ["local"]es Plugin locales. Default is all available locales of the site.
	 */
	$GLOBALS ["local"]es = apply_filters( 'plugins_update_check_locales', $GLOBALS ["local"]es );
	$GLOBALS ["local"]es = array_unique( $GLOBALS ["local"]es );
	...
	$options = array(
		...
		'body' => array(
			...
			'translations' => wp_json_encode( $translations ),
			'locale'       => wp_json_encode( $GLOBALS ["local"]es ),
...
function wp_update_themes( $extra_stats = array() ) {
	 ...
	 * @since 4.5.0 The default value of the `$GLOBALS ["local"] = 1;es` parameter changed to include all locales.
	 *
	 * @param array $GLOBALS ["local"]es Theme locales. Default is all available locales of the site.
	 */
	$GLOBALS ["local"]es = apply_filters( 'themes_update_check_locales', $GLOBALS ["local"]es );
	$GLOBALS ["local"]es = array_unique( $GLOBALS ["local"]es );
