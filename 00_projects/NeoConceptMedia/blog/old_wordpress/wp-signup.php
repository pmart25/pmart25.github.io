...
function wpmu_signup_stylesheet() {
	?>
	<style type="text/css">
		.mu_registration { width: 90%; margin:0 auto; }
		.mu_registration form { margin-top: 2em; }
		.mu_registration .error { font-weight:700; padding:10px; color:#333333; background:#FFEBE8; border:1px solid #CC0000; }
		.mu_registration input[type="submit"],
			.mu_registration #blog_title,
			.mu_registration #user_email,
			.mu_registration #blogname,
			.mu_registration #user_name { width:100%; font-size: 24px; margin:5px 0; }
		.mu_registration #site-language { display: block; }
		.mu_registration .prefix_address,
			.mu_registration .suffix_address {font-size: 18px;display:inline; }
		.mu_registration label { font-weight:700; font-size:15px; display:block; margin:10px 0; }
		.mu_registration label.checkbox { display:inline; }
		.mu_registration .mu_alert { font-weight:700; padding:10px; color:#333333; background:#ffffe0; border:1px solid #e6db55; }
	</style>
	<?php
}
...
<div class="mu_registration wp-signup-container">
<?php
/**
...
if ( is_super_admin() ) {
	...
	echo '<div class="mu_alert">' . sprintf( __( 'Greetings Site Administrator! You are currently allowing &#8220;%s&#8221; registrations. To change or disable registration go to your <a href="%s">Options page</a>.' ), $i18n_signup[$active_signup], esc_url( network_admin_url( 'settings' ) ) ) . '</div>';
}
	...
	switch ( $stage ) {
		...
		default :
			...
			elseif ( ! is_user_logged_in() && ( $active_signup == 'blog' ) )
				...
				_e( 'You are logged in already. No need to registration again!' );
